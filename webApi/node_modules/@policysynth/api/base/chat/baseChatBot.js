...
export class PsBaseChatBot {
    ...
    constructor(wsClientId, wsClients, memoryId = undefined) {
        ...
        console.log(`🔇 Base class silentMode in constructor: ${this.silentMode}`);
        
        this.wsClientSocket = wsClients.get(this.wsClientId);
        console.log(`🔌 WebSocket lookup: wsClientId=${this.wsClientId}, found=${!!this.wsClientSocket}`);
        if (this.wsClientSocket) {
            console.log(`🔌 WebSocket connection state: ${this.wsClientSocket.readyState}`);
        }
        ...
        if (!this.wsClientSocket) {
            ...
            console.error(`Available wsClientIds: ${Array.from(wsClients.keys())}`);
        }
        this.setupMemory(memoryId);
        console.log(`🔇 Base class silentMode after setupMemory: ${this.silentMode}`);
    }
    ...
    sendAgentStart(name, hasNoStreaming = true) {
        console.log(`🔵 Base class sendAgentStart called with: ${name}, hasNoStreaming: ${hasNoStreaming}`);
        
        // Ensure WebSocket connection is available
        if (!this.ensureWebSocketConnection()) {
            console.error(`🔴 Cannot send agent start message - WebSocket connection unavailable`);
            return;
        }
        ...
        try {
            console.log(`🔵 Sending WebSocket message:`, JSON.stringify(botMessage));
            this.wsClientSocket.send(JSON.stringify(botMessage));
    ...
    sendAgentCompleted(name, lastAgent = false, error = undefined) {
        console.log(`✅ Base class sendAgentCompleted called with: ${name}, lastAgent: ${lastAgent}, error: ${error}`);
        
        // Ensure WebSocket connection is available
        if (!this.ensureWebSocketConnection()) {
            console.error(`🔴 Cannot send agent completed message - WebSocket connection unavailable`);
            return;
        }
    ...
    sendAgentUpdate(message) {
        console.log(`🔄 Base class sendAgentUpdate called with: ${message}`);
        
        // Ensure WebSocket connection is available
        if (!this.ensureWebSocketConnection()) {
            console.error(`🔴 Cannot send agent update message - WebSocket connection unavailable`);
            return;
        }
    ...
    // Try to reconnect WebSocket if lost
    tryReconnectWebSocket() {
        if (this.wsClients && this.wsClientId) {
            const newSocket = this.wsClients.get(this.wsClientId);
            if (newSocket && newSocket.readyState === 1) {
                console.log(`🔄 Reconnected WebSocket for client ${this.wsClientId}`);
                this.wsClientSocket = newSocket;
                return true;
            }
        }
        console.error(`🔴 Failed to reconnect WebSocket for client ${this.wsClientId}`);
        return false;
    }

    // Validate and ensure WebSocket connection is available
    ensureWebSocketConnection() {
        if (!this.wsClientSocket || this.wsClientSocket.readyState !== 1) {
            console.log(`🔄 WebSocket connection lost, attempting to reconnect...`);
            return this.tryReconnectWebSocket();
        }
        return true;
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                for await (const part of stream) {
                    ...
                    console.log(`🔍 Content: "${part.choices[0]?.delta?.content}"`);
                    console.log(`🔍 Content type: ${typeof part.choices[0]?.delta?.content}`);
                    console.log(`🔍 Content length: ${part.choices[0]?.delta?.content?.length || 'undefined'}`);
                    ...
                    if (part.choices[0]?.delta?.content) {
                        ...
                        console.log(`🔍 Current botMessage: "${botMessage}"`);
                    }
    ...
    // need get tokens but avoid Iengineconstants TO DO
