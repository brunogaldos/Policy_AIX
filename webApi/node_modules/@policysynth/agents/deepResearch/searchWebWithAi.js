...
export class BaseSearchWebAgentWithAi extends PolicySynthAgent {
    ...
    async callSearchApi(query, numberOfResults, options = {}) {
    ...
    async getQueryResults(queriesToSearch, id, numberOfResults = 10, options = {}) {
        let searchResults = [];
        for (let q = 0; q < queriesToSearch.length; q++) {
            const generalSearchData = await this.callSearchApi(queriesToSearch[q], numberOfResults, options);
