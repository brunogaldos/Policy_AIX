...
export class GoogleSearchApi extends PolicySynthSimpleAgentBase {
    needsAiModel = false;
    async googleRequestWithRetry(fn) {
        let attempt = 1;
        ...
        while (attempt <= maxAttempts) {
            try {
                ...
                if (axios.isAxiosError(error) && error.response?.status === 429) {
                    this.logger.debug(`Got 429 on attempt #${attempt}. Will back off for ${backoffSec}s, then retry.`);
    ...
    async search(query, numberOfResults, options = {}) {
        ...
        for (let callIndex = 0; callIndex < callsNeeded; callIndex++) {
            ...
            let url = `https://www.googleapis.com/customsearch/v1?q=${encodeURIComponent(finalQuery)}&key=${process.env.GOOGLE_SEARCH_API_KEY}&cx=${process.env.GOOGLE_SEARCH_API_CX_ID}&num=${resultsToFetch}&start=${startIndex}`;
