...
export class AgentQueueManager extends PolicySynthAgentBase {
    ...
    initializeRedis() {
        ...
        this.logger.info("AgentQueueManager: Initializing Redis connection: " + redisUrl);
        const options = {
    ...
    async controlAgent(agentId, action) {
        this.logger.info(`AgentQueueManager: Controlling agent ${agentId} with action ${action}`);
        const agent = await PsAgent.findByPk(agentId, {
            include: [{ model: PsAgentClass, as: "Class" }],
        });
        if (!agent || !agent.Class) {
            this.logger.error(`AgentQueueManager: Agent or Agent Class not found for agent ${agentId}`);
            throw new Error("Agent or Agent Class not found");
        }
        const queueName = agent.Class.configuration.queueName;
        if (!queueName) {
            this.logger.error(`AgentQueueManager: Queue name not defined for agent class ${agent.Class.id}`);
            throw new Error("Queue name not defined for this agent class");
        }
        const queue = this.getQueue(queueName);
        this.logger.info(`AgentQueueManager: Adding ${action} job to queue ${queueName} for agent ${agentId}`);
    ...
    async startAgentProcessing(agentId) {
        this.logger.info(`AgentQueueManager: Starting agent processing for agent ${agentId}`);
        const agent = await PsAgent.findByPk(agentId, {
            include: [{ model: PsAgentClass, as: "Class" }],
        });
        if (!agent || !agent.Class) {
        ...
        const queueName = agent.Class.configuration.queueName;
        const queue = this.getQueue(queueName);
        this.logger.info(`AgentQueueManager: Adding start-processing job to queue ${queueName} for agent ${agentId}`);
        await queue.add("control-message", {
            type: "start-processing",
            agentId: agent.id,
        });
        this.logger.info(`AgentQueueManager: Updating agent ${agentId} status to running`);
    ...
    async clearAgentStatusMessages(agentId) {
        const agent = await PsAgent.findByPk(agentId, {
            include: [{ model: PsAgentClass, as: "Class" }],
        });
        if (agent) {
