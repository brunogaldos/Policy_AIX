...
export class AgentCostManager extends PolicySynthAgentBase {
    async getSubAgentIds(rootId) {
        ...
        while (queue.length > 0) {
            ...
            const children = await sequelize.query(`SELECT id FROM ps_agents WHERE parent_agent_id = :currentAgentId`, {
    ...
    calcCosts(mu, prices) {
        const costInNormal = ((mu.token_in_count || 0) * (prices.costInTokensPerMillion || 0)) /
            1000000.0;
        const costInCached = ((mu.token_in_cached_context_count || 0) *
            (prices.costInCachedContextTokensPerMillion || 0)) /
            1000000.0;
        const costInLong = ((mu.long_context_token_in_count || 0) *
            (prices.longContextCostInTokensPerMillion || 0)) /
            1000000.0;
        // Sum all token_out_ variants for normal context
        const totalTokenOutNormal = (mu.token_out_count || 0) +
            (mu.token_out_reasoning_count || 0) +
            (mu.token_out_audio_count || 0) +
            (mu.token_out_image_count || 0);
        const costOutNormal = (totalTokenOutNormal * (prices.costOutTokensPerMillion || 0)) / 1000000.0;
        // Sum all long_context_token_out_ variants for long context
        const totalTokenOutLong = (mu.long_context_token_out_count || 0) +
        ...
        const costOutLong = (totalTokenOutLong *
        ...
        const costInCachedLong = ((mu.long_context_token_in_cached_context_count || 0) * // Note: Using IN cached count for OUT cost calculation as per schema fields
            (prices.longContextCostInCachedContextTokensPerMillion ||
                prices.costInCachedContextTokensPerMillion ||
                0)) /
            1000000.0;
        const totalCost = costInNormal +
            costInCached +
            costInLong +
            costOutNormal +
            costOutLong +
      ...
      `, {
            ...
            return usageRows.map((row) => {
                const modelUsage = {
                    ...
                    token_in_cached_context_count: parseInt(row.token_in_cached_context_count || "0"),
                    long_context_token_in_count: parseInt(row.long_context_token_in_count || "0"),
                    long_context_token_out_count: parseInt(row.long_context_token_out_count || "0"),
                    long_context_token_in_cached_context_count: parseInt(row.long_context_token_in_cached_context_count || "0"),
                    token_out_count: parseInt(row.token_out_count || "0"),
                    token_out_reasoning_count: parseInt(row.token_out_reasoning_count || "0"),
                    token_out_audio_count: parseInt(row.token_out_audio_count || "0"),
                    token_out_image_count: parseInt(row.token_out_image_count || "0"),
                    long_context_token_out_reasoning_count: parseInt(row.long_context_token_out_reasoning_count || "0"),
                    long_context_token_out_audio_count: parseInt(row.long_context_token_out_audio_count || "0"),
                    long_context_token_out_image_count: parseInt(row.long_context_token_out_image_count || "0"),
                    agent_id: row.agent_id,
                };
                const prices = row.price_cfg;
                ...
                return {
                    ...
                    tokenInCount: (modelUsage.token_in_count || 0) +
                    ...
                    tokenInCachedContextCount: modelUsage.token_in_cached_context_count || 0,
                    longContextTokenInCount: modelUsage.long_context_token_in_count || 0,
                    longContextTokenOutCount: modelUsage.long_context_token_out_count || 0,
                    longContextTokenInCachedContextCount: modelUsage.long_context_token_in_cached_context_count || 0,
                    tokenOutCount: (modelUsage.token_out_count || 0) +
      ...
      `, {
        ...
        `, {
            ...
            usageRows.forEach((row) => {
                ...
                const prices = row.price_cfg;
                const costs = this.calcCosts(modelUsage, prices);
                agentCostMap.set(row.agent_id, (agentCostMap.get(row.agent_id) || 0) + costs.totalCost);
            });
            const агентCosts = Array.from(agentCostMap.entries())
                .map(([agentId, cost]) => ({
                agentId,
                level: agentLevelsMap.get(agentId) !== undefined
                    ? agentLevelsMap.get(agentId)
                    : -1, // Fallback level
                cost: parseFloat(cost.toFixed(2)),
            }))
                .sort((a, b) => (a.level ?? 0) - (b.level ?? 0) || a.agentId - b.agentId);
            const totalCost = агентCosts
                .reduce((sum, agent) => sum + agent.cost, 0)
                .toFixed(2);
            return { agentCosts: агентCosts, totalCost };
        }
        catch (error) {
