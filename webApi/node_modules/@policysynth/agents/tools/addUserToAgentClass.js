        ...
        for (const agentClass of agentClasses) {
            ...
            if (!hasAccess) {
                ...
                if (role === "user") {
                    ...
                    PolicySynthAgentBase.logger.info(`User ${userId} added as user to agent class ${agentClass.id}`);
                }
                else if (role === "admin") {
                    await agentClass.addAdmin(user);
                    PolicySynthAgentBase.logger.info(`User ${userId} added as admin to agent class ${agentClass.id}`);
                }
            }
            else {
                PolicySynthAgentBase.logger.info(`User ${userId} already has ${role} access to agent class ${agentClass.id}`);
            }
        }
    }
    catch (error) {
...
if (args.length !== 3) {
    PolicySynthAgentBase.logger.error("Usage: ts-node addUserToAgentClasses.ts <agentClassBaseId> <userId> <role>");
    process.exit(1);
}
