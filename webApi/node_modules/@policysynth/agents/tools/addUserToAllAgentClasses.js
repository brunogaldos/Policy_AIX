    ...
    try {
        ...
        for (const agentClass of agentClasses) {
            ...
            if (!hasUserAccess) {
                ...
                PolicySynthAgentBase.logger.info(`User ${user.email} added as user to agent class ${agentClass.id}`);
            }
            else {
                PolicySynthAgentBase.logger.info(`User ${user.email} already has user access to agent class ${agentClass.id}`);
            }
            ...
            if (!hasAdminAccess) {
                ...
                PolicySynthAgentBase.logger.info(`User ${user.email} added as admin to agent class ${agentClass.id}`);
            }
            else {
                PolicySynthAgentBase.logger.info(`User ${user.email} already has admin access to agent class ${agentClass.id}`);
            }
        }
    }
    catch (error) {
