    ...
    async loadAgentStatusFromRedis(agentId) {
        ...
        try {
            ...
            if (statusDataString) {
                ...
                this.logger.debug(`No status data found for agent ${agentId} ${psAgent.redisStatusKey}`);
            }
        }
        catch (error) {
    ...
    async setupAgentQueue() {
        if (this.agentQueueName) {
            ...
            const worker = new Worker(this.agentQueueName, async (job) => {
                try {
                    this.logger.info(`Processing job ${job.id} on worker for queue ${this.agentQueueName}`);
                    const data = job.data;
                    ...
                    this.logger.debug(`${action} agent ${loadedAgent.id} with structured answers overrides:`, JSON.stringify(structuredAnswersOverrides));
                    // 4) Setup status
                    ...
                    this.logger.error(`Error processing job ${job.id} for queue ${this.agentQueueName}`, error);
                    throw error;
                }
            }, {
