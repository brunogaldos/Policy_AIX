...
export class IngestionAgentProcessor extends BaseIngestionAgent {
    ...
    async addDocumentsToWeaviate(allDocumentSourcesWithChunks) {
        ...
        const postChunkRecursively = async (chunk, documentId, parentChunkId, allSiblingChunksIncludingMe) => {
            ...
            if (allSiblingChunksIncludingMe &&
                ...
                this.logger.info(`4. importantContextChunkIndexes ${JSON.stringify(chunk.importantContextChunkIndexes)}`);
                ...
                for (const subChunk of allSiblingChunksIncludingMe) {
                    if (subChunk.chapterIndex != chunk.chapterIndex) {
                        this.logger.info(`Bottom level loop: Processing sibling chunk ${subChunk.chapterIndex} current chunk.subChunks: ${chunk.subChunks?.map((c) => c.chapterIndex)}`);
                        ...
                        if (subChunkId) {
                            ...
                            this.logger.error(`Error: Failed to post sibling chunk ${subChunk.chapterIndex} NO CHUNK ID`);
                        }
                    }
                    else {
                        this.logger.info(`Skipping myself ${subChunk.chapterIndex} for ${chunk.chapterIndex}`);
                    }
                }
                this.logger.info(`5. importantContextChunkIndexes ${JSON.stringify(chunk.importantContextChunkIndexes)}`);
                ...
                this.logger.info(`6. importantContextChunkIndexes ${JSON.stringify(chunk.importantContextChunkIndexes)}`);
                if (chunk.importantContextChunkIndexes &&
                    chunk.importantContextChunkIndexes.length > 0) {
                    this.logger.info(`1. Adding relevant sibling chunks for ${chunkId} ${chunk.chapterIndex}`);
                    for (const sibling of allSiblingChunksWithIds) {
                        this.logger.info(`2. Adding relevant sibling chunks for ${sibling.id} ${sibling.chapterIndex}`);
                ...
                this.logger.info(`No sibling chunks to process for ${chunkId} length ${allSiblingChunksIncludingMe
                    ? allSiblingChunksIncludingMe.length
                    : -1}`);
            }
            if (chunk.subChunks) {
                for (const subChunk of chunk.subChunks) {
                    this.logger.info(`Middle Level Loop: Processing subChunk ${subChunk.chapterIndex}`);
                ...
                if (documentId) {
                    if (source.chunks) {
                        for (const chunk of source.chunks) {
                            this.logger.info(`Top Level Loop: Processing chunk ${chunk.chapterIndex} for ${source.url}`);
                            try {
                                await postChunkRecursively(chunk, documentId, undefined, source.chunks);
                            }
                            catch (error) {
                                this.logger.error(`Failed to post chunk for document ${source.url}:\n${JSON.stringify(chunk)}`, error);
                            }
                        }
                    }
                }
    ...
    async classifyDocuments(allDocumentSourcesWithChunks) {
        ...
        for (const category of this.dataLayout.categories) {
            ...
            this.logger.info(`Ranking by relevance within the ${category} category (${eloRatingFieldName})`);
    ...
    updateCachedFilesAndMetadata(relativePath, url, data, contentType, lastModifiedOnServer) {
        ...
        this.logger.info(`Cached file ${relativePath} ${JSON.stringify(this.fileMetadata[fileId], null, 2)}`);
    }
        ...
        for (const url of urls) {
            try {
                ...
                if (metadata) {
                    ...
                    this.logger.info(`-----> ${fetchedLastModified.getTime()} === ${cachedLastModified.getTime()}`);
