...
YOUR EVALUATION: `);
    ...
    async fetchLlmChunkingStrategy(data, review, lastJson) {
        ...
        this.logger.info(`JSON strategy: ${JSON.stringify(lastChunkingStrategyJson, null, 2)}`);
    ...
    async splitDocumentIntoChunks(data, isSubChunk = false, totalLinesInChunk) {
        this.logger.info(`Splitting document into chunks...(isSubChunk: ${isSubChunk}) (totalLinesInChunk: ${totalLinesInChunk})`);
        if (!isSubChunk) {
        ...
        while (!validated && retryCount < this.maxSplitRetries) {
            ...
            let dataWithLineNumber = data
                ...
                .join("\n");
            if (isSubChunk)
                this.logger.info(`Sub Chunk Data with line numbers:\n`);
            else
                this.logger.info(`Chunk Data with line numbers:\n`);
            ...
            try {
                ...
                this.logger.info(`Strategy validatation results: ${chunkingStrategyReview.trim().toUpperCase().indexOf("PASSES") > -1}`);
                if (lastChunkingStrategyJson &&
                    ...
                    for (let i = 0; i < lastChunkingStrategyJson.length; i++) {
                        this.logger.info(`Processing chunk ${i + 1} of ${lastChunkingStrategyJson.length}`);
                        ...
                        this.logger.info(`Start line for chunk ${i + 1}: ${startLine} endline: ${endLine}`);
                        const chunkSize = endLine - startLine + 1;
                        this.logger.info(`Calculated chunk size for chunk ${i + 1}: ${chunkSize} lines`);
                        // Check if chunk size calculation is logical; otherwise, adjust
                        if (chunkSize <= 0) {
                            this.logger.error(`Warning: Calculated chunk size for chunk ${i + 1} is non-positive, adjusting...`);
                        }
                        ...
                        if (chunkSize > this.maxChunkLinesLength) {
                            ...
                            this.logger.info(`Creating subchunks startline ${startLine}, endline ${endLine}, totalLinesInOversizedChunk ${totalLinesInOversizedChunk}`);
                            const subChunks = await this.splitDocumentIntoChunks(oversizedChunkContent, true, totalLinesInOversizedChunk);
                            this.logger.info(`Completed processing subchunks for chunk ${i + 1}. Received ${subChunks ? subChunks.length : 0} subchunks.`);
                            strategy.subChunks = [];
                            strategy.subChunks.push(...subChunks);
                        }
                        else {
                            this.logger.info(`Chunk ${i + 1} is within size limits (${chunkSize} lines), no need for subchunking.`);
                ...
                if (!validated) {
                    retryCount++;
                    this.logger.warn(`Validation attempt failed, retrying... (${retryCount})`);
                }
                ...
                if (validated && lastChunkingStrategyJson) {
                    ...
                    if (normalizedAggregatedData !== normalizedOriginalData) {
                        ...
                        this.logger.error(`Validation failed: Normalized chunk data does not match the normalized original data. ${normalizedAggregatedData.length} !== ${normalizedOriginalData.length}`);
                        validated = false;
                    }
                    else {
                        this.logger.info("Validation passed: Normalized chunk data matches the normalized original data.");
                    }
                }
            }
            catch (e) {
