...
export class PsYourPrioritiesConnector extends PsBaseIdeasCollaborationConnector {
    ...
    constructor(connector, connectorClass, agent, memory = undefined, startProgress = 0, endProgress = 100) {
        ...
        this.logger.info(`Your Priorities Connector created with group ID: ${groupId} serverBaseUrl: ${this.serverBaseUrl}`);
    }
            ...
            try {
                ...
                if (axios.isAxiosError(error)) {
                    ...
                    if (error.code === "ECONNREFUSED" ||
                        ...
                        this.logger.error(`${error.code}: Retry ${connectionErrorsSoFar}/${MAX_CONNECTION_RETRIES} in ${RETRY_DELAY}ms`);
                    ...
                    // 5xx errors
                        ...
                        serverErrorsSoFar++;
                        this.logger.error(`5xx Server Error: Retry ${serverErrorsSoFar}/${MAX_5XX_RETRIES} in ${RETRY_DELAY}ms`);
    ...
    async login() {
        if (!process.env.PS_TEMP_AGENTS_FABRIC_GROUP_API_KEY) {
            if (!this.user) {
                ...
                try {
                    const response = await this.requestWithRetry(() => axios.post(`${this.serverBaseUrl}/users/login`, loginData, {
    ...
    async getGroupPosts(groupId) {
        ...
        while (isMorePosts) {
            ...
            try {
                const response = await this.requestWithRetry(() => axios.get(url, {
    ...
    async postPoint(groupId, postId, userId, value, content) {
        await this.login();
        const pointData = {
            post_id: postId,
            user_id: userId,
        ...
        try {
            const response = await this.requestWithRetry(() => axios.post(`${this.serverBaseUrl}/points/${groupId}${this.agentFabricUserId ? `?agentFabricUserId=${this.agentFabricUserId}` : ""}`, pointData, {
                headers: this.getHeaders(),
            }));
    ...
    async vote(postId, value) {
        ...
        try {
            const response = await this.requestWithRetry(() => axios.post(`${this.serverBaseUrl}/posts/${postId}/endorse${this.agentFabricUserId
    ...
    async post(groupId, name, structuredAnswersData, imagePrompt, imageLocalPath = undefined) {
        ...
        if (imageLocalPath) {
            ...
            try {
                const imageUploadResponse = await this.requestWithRetry(() => axios.post(`${this.serverBaseUrl}/images${this.agentFabricUserId
                    ? `?agentFabricUserId=${this.agentFabricUserId}`
                    : ""}`, imageForm, {
                    headers: {
                        ...this.getHeaders(),
        ...
        try {
            ...
            const postResponse = await this.requestWithRetry(() => axios.post(`${this.serverBaseUrl}/posts/${groupId}${this.agentFabricUserId
                ? `?agentFabricUserId=${this.agentFabricUserId}`
    ...
    async generateImageWithAi(groupId, prompt) {
        ...
        try {
            ...
            const startResponse = await this.requestWithRetry(() => axios.post(`${this.serverBaseUrl}/groups/${groupId}/start_generating/ai_image`, {
                imageType: "logo",
                prompt: prompt,
            }, {
                headers: this.getHeaders(),
            ...
            while (isGenerating) {
                pollResponse = await this.requestWithRetry(() => axios.get(`${this.serverBaseUrl}/groups/${groupId}/${jobId}/poll_for_generating_ai_image`, { headers: this.getHeaders() }));
                ...
                if (pollResponse.data.data.imageId) {
                    ...
                    isGenerating = false;
                    this.logger.error("Error generating AI image:", pollResponse.data.data.error);
                }
                ...
                if (isGenerating) {
                    ...
                    if (Date.now() - startTime > AI_IMAGE_GENERATION_TIMEOUT_MS) {
                        this.logger.info("AI image generation timed out after 2 minutes. Skipping image...");
