import { GoogleGenAI, FunctionCallingConfigMode, HarmBlockThreshold, HarmCategory, } from "@google/genai";
...
export class GoogleGeminiChat extends BaseChatModel {
    ...
    modelName;
    constructor(config) {
        super(config, config.modelName || "gemini-2.0-flash", config.maxTokensOut || 16_000);
    ...
    async logTokens(tokensIn, tokensOut, cached) {
        if (!process.env.DEBUG_TOKENS_COUNTS_TO_CSV_FILE)
            return;
        await appendFile("/tmp/geminiTokenDebug.csv", `${this.modelName},${tokensIn},${tokensOut},${cached}\n`);
    }
    ...
    assertGeminiNotBlocked(response) {
        if (!response.candidates?.length) {
            const pf = response.promptFeedback;
            if (pf?.blockReason) {
                // The prompt was blocked; include the reason in the error
                throw new Error(`Response was blocked via prompt: ${pf.blockReason} `
        ...
        if (candidate.finishReason === 'SAFETY') {
            throw new Error(`Response was blocked via safety: `
                + (candidate.safetyRatings?.map(r => `${r.category}=${r.probability}`).join(', ') || ''));
        }
    }
    /* ---------- main generate() entry‑point ---------- */
    async generate(messages, streaming = false, streamingCallback, media, tools, toolChoice = "auto", allowedTools) {
