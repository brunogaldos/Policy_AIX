...
let PsOperationsManager = class PsOperationsManager extends PsBaseWithRunningAgentObserver {
    ...
    async connectedCallback() {
        ...
        this.addEventListener("agent-deleted", this.getAgent);
    }
    ...
    renderDetailedCostsTab() {
      ...
      <vaadin-grid .items="${this.detailedCosts}">
        ...
        <vaadin-grid-sort-column
          ...
          .renderer="${(root, _column, rowData) => {
            root.textContent = `${YpFormattingHelpers.number(rowData.item.longContextTokenInCount || 0)}`;
        }}"
        ></vaadin-grid-sort-column>
        <vaadin-grid-sort-column
          path="longContextIn"
          ...
          .renderer="${(root, _column, rowData) => {
            root.textContent = `${YpFormattingHelpers.number(rowData.item.longContextTokenOutCount || 0)}`;
        }}"
        ></vaadin-grid-sort-column>
        <vaadin-grid-sort-column
          ...
          .renderer="${(root, _column, rowData) => {
            root.textContent = `${YpFormattingHelpers.number(rowData.item.tokenInCachedContextCount || 0)}`;
        }}"
        ></vaadin-grid-sort-column>
        <vaadin-grid-sort-column
          path="costIn"
