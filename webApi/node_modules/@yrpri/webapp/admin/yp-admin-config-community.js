...
let YpAdminConfigCommunity = class YpAdminConfigCommunity extends YpAdminConfigBase {
    ...
    async _openClone() {
        ...
        try {
            ...
            const newCommunity = (await window.serverApi.apiAction(`/api/communities/${this.collection.id}/clone`, "POST", {}));
    ...
    async _openTemplatesDialog() {
        const domainId = this.collection.Domain?.id;
        if (!domainId)
            return;
        try {
            this.templates = await window.adminServerApi.getCommunityTemplates(domainId);
            this.requestUpdate();
    ...
    async _cloneTemplate(template, event) {
        ...
        try {
            const newCommunity = (await window.serverApi.apiAction(`/api/communities/${template.id}/clone`, "POST", {}));
            ...
            window.appGlobals.activity("error", "cloneCommunityFromTemplate", err.message);
        ...
        <md-list slot="content">
          ${this.templates && this.templates.length > 0
            ? this.templates.map((t) => html `
                  <md-list-item
                    @click="${(e) => this._cloneTemplate(t, e)}"
...
__decorate([
    property({ type: Array })
], YpAdminConfigCommunity.prototype, "templates", void 0);
__decorate([
    property({ type: Boolean })
], YpAdminConfigCommunity.prototype, "cloning", void 0);
__decorate([
    property({ type: Number })
], YpAdminConfigCommunity.prototype, "cloningTemplateId", void 0);
